...
⚡ يرجى تحضير الطلب والتوصيل في أسرع وقت`;
  ...
  async sendCustomerInvoice(customerPhone: string, orderData: any, pdfBuffer: Buffer): Promise<boolean> {
    if (!this.isConnected || !this.socket) {
      console.log('⚠️ WhatsApp not connected - cannot send customer invoice');
      return false;
    }

    try {
      console.log(`📄 Sending customer invoice via Baileys to ${customerPhone}`);
...
شكراً لاختيارك PAKETY للتوصيل السريع! 🙏`;
      ...
      // Send text message first
      await this.socket.sendMessage(formattedNumber, { text: customerMessage });
      ...
      const mediaMessage = await prepareWAMessageMedia(
        { 
          document: pdfBuffer, 
          mimetype: 'application/pdf',
          fileName: `فاتورة_طلب_${orderData.orderId}.pdf`
        },
        { upload: this.socket.waUploadToServer }
      );

      const documentMessage = generateWAMessageFromContent(
        formattedNumber,
        {
          documentMessage: {
            ...mediaMessage.document,
            caption: `📄 فاتورة طلب رقم ${orderData.orderId}`
          }
        },
        { userJid: this.socket.user?.id }
      );

      await this.socket.relayMessage(formattedNumber, documentMessage.message!, {});
      
      console.log(`✅ Customer invoice sent successfully to ${customerPhone}`);
      return true;
    } catch (error) {
      console.error(`❌ Failed to send customer invoice to ${customerPhone}:`, error);
      return false;
    }
  }
  ...
  async sendAdminInvoice(adminPhone: string, orderData: any, pdfBuffer: Buffer): Promise<boolean> {
    if (!this.isConnected || !this.socket) {
      console.log('⚠️ WhatsApp not connected - cannot send admin invoice');
      return false;
    }

    try {
      console.log(`📄 Sending admin invoice via Baileys to ${adminPhone}`);
      
      const formattedNumber = this.formatPhoneNumber(adminPhone);
...
⚡ يرجى تحضير الطلب والتوصيل في أسرع وقت`;
      ...
      // Send text message first
      await this.socket.sendMessage(formattedNumber, { text: adminMessage });
      ...
      const mediaMessage = await prepareWAMessageMedia(
        { 
          document: pdfBuffer, 
          mimetype: 'application/pdf',
          fileName: `فاتورة_طلب_${orderData.orderId}_للإدارة.pdf`
        },
        { upload: this.socket.waUploadToServer }
      );
      ...
      const documentMessage = generateWAMessageFromContent(
        formattedNumber,
        {
          documentMessage: {
            ...mediaMessage.document,
            caption: `📄 فاتورة طلب رقم ${orderData.orderId} - للإدارة`
          }
        },
        { userJid: this.socket.user?.id }
      );
      ...
      console.log(`✅ Admin invoice sent successfully to ${adminPhone}`);
      return true;
    } catch (error) {
      console.error(`❌ Failed to send admin invoice to ${adminPhone}:`, error);
      return false;
    }
  }
}
