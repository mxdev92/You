  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification('✅ تم إرسال رمز التحقق عبر SMS بنجاح', 'success');
      } else {
        // Show error message for failed OTP sending
        showNotification('فشل في إرسال رمز التحقق عبر SMS. يرجى المحاولة مرة أخرى', 'error');
      }
    } catch (error) {
      if (error.name === 'AbortError') {
        // Set OtpSent to true even on timeout since backend likely processed it
        setOtpSent(true);
        showNotification('✅ تم إرسال رمز التحقق عبر SMS', 'success');
      } else {
  ...
  const verifyOTP = async () => {
    ...
    try {
      ...
      if (response.ok && data.valid) {
        showNotification('تم التحقق من رقم الهاتف بنجاح', 'success');
                        ...
                        <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="text-sm font-medium text-green-700" style={{ fontFamily: 'Cairo, system-ui, sans-serif' }}>
                              سنرسل لك رمز التأكيد عبر رسالة نصية
                            </span>
                          </div>
                        </div>
                        <Input
